...
_RULES = (
    ...
    "2. ENGLISH PRONUNCIATION for tech terms. Code identifiers, framework/library "
    "names and English technical words must be written with TURKISH LETTERS that "
    "mimic their ENGLISH pronunciation, so the Turkish TTS voice says them like an "
    "English speaker — NOT a letter-by-letter Turkish reading. Attach Turkish "
    "suffixes to the respelled form. Examples:\n"
    "   React → \"riekt\"   useState → \"yus steyt\"   useEffect → \"yus ifekt\"\n"
    "   component → \"komponent\"   props → \"props\"   state → \"steyt\"\n"
    "   hook → \"huk\"   array → \"erey\"   object → \"obcekt\"   async → \"eysink\"\n"
    "   await → \"ıveyt\"   Promise → \"pramıs\"   function → \"fankşın\"   Python → \"payton\"\n"
    "   queue → \"kyu\"   cache → \"keş\"   deploy → \"diploy\"   commit → \"kamit\"\n"
    "Generalize to ANY English tech term/identifier. Inflect: \"riekt'te\", "
    "\"steyt'i\", \"komponent'ler\".\n"
